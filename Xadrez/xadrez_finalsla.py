...
def mover_peca(tabuleiro: list, origem: tuple, destino: tuple,
               turno: str, capturadas: list) -> str:
    """
    Tenta executar um lance.
    Retorna: 'invalido', 'ok' ou 'rei_capturado'
    Demonstra: if/elif/else, .upper(), .isupper(), append(), str
    """
    lo, co = origem
    ld, cd = destino
    ...
    peca_destino = tabuleiro[ld][cd]
    rei_capturado: bool = False
    if peca_destino != VAZIO:
        capturadas.append(peca_destino)        # append() à lista de capturadas
        if peca_destino.upper() == 'K':        # verifica se é o rei
            rei_capturado = True
...
def jogar() -> None:
    ...
    while jogo_ativo:                   # while condicional
        ...
        resultado = mover_peca(tabuleiro, origem, destino, turno, capturadas_turno)

        if resultado == 'rei_capturado':
            historico.append(entrada)
            exibir_tabuleiro(tabuleiro, capturadas_brancas, capturadas_pretas)
            vencedor: str = turno.upper()          # str: cor do vencedor
            print('  ' + '=' * 44)
            print('  ★  PARTIDA ENCERRADA  ★')
            print(f'  Vitória das {vencedor}!')
            print('  ' + '=' * 44)
            jogo_ativo = False                     # encerra o while
